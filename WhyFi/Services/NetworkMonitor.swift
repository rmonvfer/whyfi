...
final class NetworkMonitor {
    ...
    private let captivePortalService = CaptivePortalService()

    private var pollingTask: Task<Void, Never>?
    private var captivePortalCheckCounter = 0
    ...
    private func collectMetrics() async {
        ...
        captivePortalCheckCounter += 1
        if captivePortalCheckCounter >= 10 || captivePortalStatus == .unknown {
            captivePortalCheckCounter = 0
            let portalStatus = await captivePortalService.checkForCaptivePortal()
            captivePortalStatus = portalStatus
            if case .detected(let url) = portalStatus {
                captivePortalLoginURL = url
            } else {
                captivePortalLoginURL = nil
            }
        }
